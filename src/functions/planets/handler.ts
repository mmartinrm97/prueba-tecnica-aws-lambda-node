...
import { PlanetStarWars } from 'src/interfaces/star-wars';
import dynamoDBClient from 'src/dynamodb/useDynamoDB';
import { planetTranslation } from 'src/utils/translations';

const TABLE_NAME = 'PlanetsTable'
const MODEL_ROUTE = 'planets'
...
export const main: APIGatewayProxyHandler = async (event: APIGatewayProxyEvent) => {
    try {
        ...
        const response = await useApi<PlanetStarWars>(`/${MODEL_ROUTE}/${id}`)

        // Traducir los atributos de inglés a español
        const translatedData = translateAttributes(response.data, planetTranslation);
