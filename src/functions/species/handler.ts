...
import { SpecieStarWars } from 'src/interfaces/star-wars';
import dynamoDBClient from 'src/dynamodb/useDynamoDB';
import { specieTranslation } from 'src/utils/translations';

const TABLE_NAME = 'SpeciesTable'
const MODEL_ROUTE = 'species'
...
export const main: APIGatewayProxyHandler = async (event: APIGatewayProxyEvent) => {
    try {
        ...
        const response = await useApi<SpecieStarWars>(`/${MODEL_ROUTE}/${id}`)

        // Traducir los atributos de inglés a español
        const translatedData = translateAttributes(response.data, specieTranslation);
