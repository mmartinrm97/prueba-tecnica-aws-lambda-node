...
import { StarshipStarWars } from 'src/interfaces/star-wars';
import dynamoDBClient from 'src/dynamodb/useDynamoDB';
import { starshipTranslation } from 'src/utils/translations';

const TABLE_NAME = 'StarshipsTable'
const MODEL_ROUTE = 'starships'
...
export const main: APIGatewayProxyHandler = async (event: APIGatewayProxyEvent) => {
    try {
        ...
        const response = await useApi<StarshipStarWars>(`/${MODEL_ROUTE}/${id}`)

        // Traducir los atributos de inglés a español
        const translatedData = translateAttributes(response.data, starshipTranslation);
